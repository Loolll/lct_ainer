...
from models import NtoPaperForm, Georaphy
from store import nto_paper as store
...
async def load(pool: asyncpg.Pool):
    with open('/datasets/data_src/nto_paper.json') as file:
    ...
    async def _load(obj):
        ...
        with suppress(asyncpg.UniqueViolationError):
            await store.create_nto_paper(
                pool, form=NtoPaperForm(
                    id=id,
                    address=address,
        ...
        if not i % 25 or i == rows:
            logging.info(f"LOADED NTO PAPER {i/rows:.2%}")
